...
def DeepSupervisionLoss(out, targets):
...
##此处需要修改
    ...
    criterion_pred = BceDiceLoss_lapulasi(torch.abs(conv_lapulasi(pred)).view(pred.size(0),-1).data,True)
    loss_pred = criterion_pred(pred, masks)

    masks = F.interpolate(masks, scale_factor=0.5, mode='bilinear', align_corners=True)
    criterion0 =BceDiceLoss_lapulasi(torch.abs(conv_lapulasi(pr0)).view(pred.size(0),-1).data,True)
    loss0 = criterion0(pr0, masks)


    masks = F.interpolate(masks, scale_factor=0.5, mode='bilinear', align_corners=True)
    criterion1 =BceDiceLoss_lapulasi(torch.abs(conv_lapulasi(pr1)).view(pred.size(0),-1).data,True)
    loss1 = criterion1(pr1, masks)

    masks = F.interpolate(masks, scale_factor=0.5, mode='bilinear', align_corners=True)
    criterion2 =BceDiceLoss_lapulasi(torch.abs(conv_lapulasi(pr2)).view(pred.size(0),-1).data,True)
    loss2 = criterion2(pr2, masks)



    masks = F.interpolate(masks, scale_factor=0.5, mode='bilinear', align_corners=True)
    criterion3 =BceDiceLoss_lapulasi(torch.abs(conv_lapulasi(pr3)).view(pred.size(0),-1).data,True)
    ...
    return loss_pred +loss0+loss1+loss2+loss3  +loss5 +loss6
